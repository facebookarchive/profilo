...
auto is_quick_resolution_stub(
    ...
    uintptr_t runtime,
    uintptr_t thread) {
  uint64_t class_linker = Read8(AccessField(runtime, 464UL));
  uintptr_t entry_points = AccessField(AccessField(thread, 128UL), 312UL);
  return (
      (Read8(AccessField(class_linker, 376UL)) == entry_point) ||
      (Read8(AccessField(entry_points, 752UL)) == entry_point));
}
...
auto is_resolved(uintptr_t cls) {
  uintptr_t status = AccessField(cls, 112UL);
...
auto find_oat_class(uintptr_t cls) {
  ...
  uint64_t oat_dex_file = Read8(AccessField(dex_file, 152UL));
  if (((oat_dex_file == 0UL) ||
...
auto runtime_is_aot_compiler(uintptr_t runtime, uintptr_t instance) {
  uint64_t jit =
      Read8(AccessField(AccessField(AccessField(runtime, 512UL), 0UL), 0UL));
  bool use_jit_compilation = ((jit != 0UL) && Read1(AccessField(jit, 376UL)));
  uint64_t compiler_callbacks = Read8(AccessField(runtime, 120UL));
...
auto get_oat_method(
  ...
  if ((Read1(AccessField(oat_file, 80UL)) ||
       ((runtime_current == 0UL) ||
...
auto get_frame_size(
  ...
  uintptr_t callee_save_infos = AccessField(runtime_obj, 72UL);
