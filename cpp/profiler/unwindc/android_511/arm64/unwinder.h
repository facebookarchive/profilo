...
auto get_method_trace_id(uintptr_t method) {
  ...
  dex_id = dex_id;
  uint32_t method_id = Read4(AccessField(method, 28UL));
...
auto get_method_name(uintptr_t method) {
  auto cls = get_declaring_class(method);
  auto dexfile = get_class_dexfile(cls);
  uint32_t dex_method_index = Read4(AccessField(method, 28UL));
  uintptr_t method_id =
...
auto get_method_access_flags(uintptr_t method) {
  uint32_t access_flags = Read4(AccessField(method, 20UL));
  return access_flags;
}
...
auto is_quick_resolution_stub(
    ...
    uintptr_t runtime,
    uintptr_t thread) {
  uint64_t class_linker = Read8(AccessField(runtime, 376UL));
  uintptr_t entry_points = AccessField(AccessField(thread, 120UL), 320UL);
  return (
      (Read8(AccessField(class_linker, 464UL)) == entry_point) ||
...
auto is_quick_to_interpreter_bridge(
  ...
  uintptr_t entry_points = AccessField(AccessField(thread, 120UL), 320UL);
  return (
      (Read8(AccessField(class_linker, 496UL)) == entry_point) ||
...
auto get_frame_size(
  ...
  uintptr_t callee_save_infos = AccessField(runtime_obj, 76UL);
