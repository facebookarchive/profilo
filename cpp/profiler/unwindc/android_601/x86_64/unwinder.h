...
auto get_method_trace_id(uintptr_t method) {
  ...
  dex_id = dex_id;
  uint32_t method_id = Read4(AccessField(method, 20UL));
...
auto get_method_name(uintptr_t method) {
  auto cls = get_declaring_class(method);
  auto dexfile = get_class_dexfile(cls);
  uint32_t dex_method_index = Read4(AccessField(method, 20UL));
  uintptr_t method_id =
...
auto get_class_descriptor(uintptr_t cls) {
  auto dexfile = get_class_dexfile(cls);
  uint32_t typeidx = Read4(AccessField(cls, 92UL));
  uintptr_t typeid_ =
...
auto get_method_access_flags(uintptr_t method) {
  uint32_t access_flags = Read4(AccessField(method, 12UL));
  return access_flags;
}
...
auto is_quick_resolution_stub(
    ...
    uintptr_t runtime,
    uintptr_t thread) {
  uint64_t class_linker = Read8(AccessField(runtime, 392UL));
  uintptr_t entry_points = AccessField(AccessField(thread, 128UL), 272UL);
  return (
      (Read8(AccessField(class_linker, 424UL)) == entry_point) ||
      (Read8(AccessField(entry_points, 624UL)) == entry_point));
}
...
auto is_quick_to_interpreter_bridge(
  ...
  uintptr_t entry_points = AccessField(AccessField(thread, 128UL), 272UL);
  return (
      (Read8(AccessField(class_linker, 448UL)) == entry_point) ||
      (Read8(AccessField(entry_points, 632UL)) == entry_point));
}
...
auto is_quick_generic_jni_stub(
  ...
  uintptr_t entry_points = AccessField(AccessField(thread, 128UL), 272UL);
  return (
      (Read8(AccessField(class_linker, 440UL)) == entry_point) ||
      (Read8(AccessField(entry_points, 416UL)) == entry_point));
}

auto get_quick_entry_point_from_compiled_code(uintptr_t method) {
  uintptr_t ptr_fields = AccessField(method, 28UL);
  uint64_t entry_point = Read8(AccessField(ptr_fields, 16UL));
...
auto get_quick_frame_info_from_entry_point(uintptr_t entry_point) {
  ...
  return AccessField(oat_method_header, 12UL);
}
...
auto unwind(unwind_callback_t __unwind_callback, void* __unwind_data) {
  ...
  uint64_t generic_jni_trampoline =
      Read8(AccessField(AccessField(tls, 272UL), 416UL));
  while ((mstack != 0UL)) {
