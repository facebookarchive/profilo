...
inline const uint8_t* OatFile::Begin() const {
  CHECK(begin_ != nullptr);
  return begin_;
}

inline const uint8_t* OatFile::End() const {
  CHECK(end_ != nullptr);
  return end_;
}

inline const uint8_t* OatFile::BssBegin() const {
  return bss_begin_;
}

inline const uint8_t* OatFile::BssEnd() const {
  return bss_end_;
}
// END FB
