...
DeviceTrace = collections.namedtuple(
    "DeviceTrace", ["file_name", "full_path", "modified_time", "size"]
)
...
def _get_file_modified_unix_time(package, path):
    ...
    command = (
        list(_ADB_CMD_BASE)
        + [package]
        + list(_GET_FILE_MODIFIED_UNIX_TIME_CMD)
        + [path]
    )
...
def _validate_trace(package, data_dir_path, file_path):
    ...
    """
    full_path = "/data/data/{package}/{path}".format(package=package, path=file_path)
    ...
    if (o_out and _NO_SUCH_FILE in o_out) or (o_err and _NO_SUCH_FILE in o_err):
...
def _create_trace(package, data_dir_path, file_path):
    ...
    modified_time = datetime.datetime.fromtimestamp(
        _get_file_modified_unix_time(package, full_path)
    )

    command = list(_ADB_CMD_BASE) + [package] + list(_GET_FILE_SIZE_CMD) + [full_path]
    ...
    return DeviceTrace(
        file_name=file_name, full_path=full_path, modified_time=modified_time, size=size
    )
...
def list_traces(package):
    if not _check_profilo_dir_exists(package):
        print(
            "Profilo directory doesn't exist, looked for",
            _PROFILO_DIR,
            "inside package internal storage",
            file=sys.stderr,
        )
        return []
